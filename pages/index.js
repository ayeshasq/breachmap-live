...
// ─── KNOWLEDGE BASE (unchanged from previous version) ────────────────────────
const ATTACK_DATA = {
  'DDoS': {
...
function DonutChart({ data }) {
  ...
  let cumulative = 0;
  const cx = 54, cy = 54, r = 40, stroke = 14;
  ...
  const slices = entries.map(([key, val]) => {
    ...
    cumulative += pct;
    return { key, val, pct, offset, rotation, color: ATTACK_COLORS[key] || '#4a5568' };
  });
  return (
    <div style={{ display:'flex', gap:'16px', alignItems:'center' }}>
      <svg width={108} height={108}>
        ...
        <text x={cx} y={cy - 4} textAnchor="middle" fill="#e2e8f0" fontSize="14" fontFamily="Orbitron" fontWeight="900">{total}</text>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
      {data.map((item, i) => (
        <div key={item.code} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
          ...
          <span style={{ color:'#718096', fontSize:'10px', width:'100px', flexShrink:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.name}</span>
          <div style={{ flex:1, height:'6px', background:'rgba(0,200,255,0.08)', borderRadius:'3px', overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${(item.count/max)*100}%`, background:`linear-gradient(90deg, #00c8ff, #7b2dff)`, borderRadius:'3px', boxShadow:'0 0 8px rgba(0,200,255,0.5)', transition:'width 0.6s ease' }} />
          </div>
          <span style={{ color:'#00c8ff', fontSize:'10px', fontFamily:'Orbitron', width:'28px', textAlign:'right', flexShrink:0 }}>{item.count}</span>
        </div>
      ))}
    </div>
  );
...
export default function Home() {
  ...
  const [globeMode, setGlobeMode] = useState(true);         // ← toggle state
  const [showStats, setShowStats] = useState(false);        // ← stats panel toggle

  // Analytics state
  const [targetCounts, setTargetCounts]   = useState({});   // code → count
  const [sourceCounts, setSourceCounts]   = useState({});   // code → count
  const [typeCounts, setTypeCounts]       = useState({});   // type → count
  const [apmHistory, setApmHistory]       = useState(Array(30).fill(0)); // attacks-per-minute history
  const apmBucketRef = useRef(0);                           // current-bucket counter

  const mapRef            = useRef(null);
  const mapInstanceRef    = useRef(null);
  const attackLayersRef   = useRef([]);
  const echoLayersRef     = useRef([]);                      // fading echo trails
  const clusterCountRef   = useRef({});                      // target code → recent hit count
  const clusterTimerRef   = useRef({});
  const spinRef           = useRef(null);
  const userInteractingRef= useRef(false);
  const heatmapDataRef    = useRef({});                      // code → intensity 0-1
  ...
  // ── Heatmap helpers ──────────────────────────────────────────────────────
  const updateHeatmap = useCallback((targetCode) => {
    const map = mapInstanceRef.current;
    if (!map || !map.getSource('heatmap-source')) return;
    heatmapDataRef.current[targetCode] = Math.min((heatmapDataRef.current[targetCode] || 0) + 0.15, 1.0);

    const features = countries.map(c => ({
      type: 'Feature',
      geometry: { type:'Point', coordinates: c.coords },
      properties: { intensity: heatmapDataRef.current[c.code] || 0, code: c.code }
    }));
    map.getSource('heatmap-source').setData({ type:'FeatureCollection', features });

    // Decay over time
    setTimeout(() => {
      heatmapDataRef.current[targetCode] = Math.max((heatmapDataRef.current[targetCode] || 0) - 0.08, 0);
      if (map.getSource('heatmap-source')) {
        const feats = countries.map(c => ({
          type:'Feature', geometry:{ type:'Point', coordinates:c.coords },
          properties:{ intensity: heatmapDataRef.current[c.code] || 0 }
        }));
        map.getSource('heatmap-source').setData({ type:'FeatureCollection', features:feats });
      }
    }, 8000);
  }, []);

  // ── Cluster explosion ────────────────────────────────────────────────────
  const triggerClusterExplosion = useCallback((targetCoords, color) => {
    const map = mapInstanceRef.current;
    if (!map) return;
    const uid = `cluster-${Date.now()}`;
    map.addSource(uid, { type:'geojson', data:{ type:'Feature', geometry:{ type:'Point', coordinates:targetCoords } } });
    ...
    const animate = (now) => {
      ...
      if (map.getLayer(uid)) {
        map.setPaintProperty(uid,'circle-radius', eased*60);
        map.setPaintProperty(uid,'circle-stroke-opacity', (1-t)*0.9);
        map.setPaintProperty(uid,'circle-stroke-width', 3 - t*2);
      }
      if (t < 1) requestAnimationFrame(animate);
      else { if(map.getLayer(uid)) map.removeLayer(uid); if(map.getSource(uid)) map.removeSource(uid); }
    };
    requestAnimationFrame(animate);
    // Second ring delayed
    setTimeout(() => {
      const uid2 = `cluster2-${Date.now()}`;
      if (!map.getStyle()) return;
      map.addSource(uid2, { type:'geojson', data:{ type:'Feature', geometry:{ type:'Point', coordinates:targetCoords } } });
      map.addLayer({ id:uid2, type:'circle', source:uid2, paint:{ 'circle-radius':0, 'circle-color':'transparent', 'circle-stroke-color':color, 'circle-stroke-width':2, 'circle-stroke-opacity':0.6, 'circle-pitch-alignment':'map' } });
      const s2 = performance.now();
      const anim2 = (now) => {
        const t = Math.min((now-s2)/1000, 1);
        if (map.getLayer(uid2)) { map.setPaintProperty(uid2,'circle-radius',t*90); map.setPaintProperty(uid2,'circle-stroke-opacity',(1-t)*0.6); }
        if (t < 1) requestAnimationFrame(anim2);
        else { if(map.getLayer(uid2)) map.removeLayer(uid2); if(map.getSource(uid2)) map.removeSource(uid2); }
      };
  ...
  const addAttackToMap = useCallback((attack) => {
    ...
    const animatePulse = (startT, ring) => {
      const dur=1200, delay=ring*400;
      const tick = (now) => {
        const t=Math.min((now-startT-delay)/dur,1);
        ...
        else if(ring<2)animatePulse(startT,ring+1);
      };
    ...
    const animateDraw = (now) => {
      ...
      const vis=fullArc.slice(0,Math.max(2,Math.floor(eased*totalSteps)));
      const arcData={type:'Feature',geometry:{type:'LineString',coordinates:vis}};
      if(map.getSource(lineId))map.getSource(lineId).setData(arcData);
      if(map.getSource(glowId))map.getSource(glowId).setData(arcData);
      if(map.getSource(dotId))map.getSource(dotId).setData({type:'Feature',geometry:{type:'Point',coordinates:vis[vis.length-1]}});
      if(progress<1)requestAnimationFrame(animateDraw);
        ...
        // ── CLUSTER CHECK ──────────────────────────────────────────────────
        const tc = attack.target.code;
        clusterCountRef.current[tc] = (clusterCountRef.current[tc]||0)+1;
        if(clusterTimerRef.current[tc]) clearTimeout(clusterTimerRef.current[tc]);
        clusterTimerRef.current[tc] = setTimeout(()=>{ clusterCountRef.current[tc]=0; },4000);
        if(clusterCountRef.current[tc]>=3) {
          triggerClusterExplosion(attack.target.coords, color);
          clusterCountRef.current[tc]=0;
        }

        // ── ECHO TRAIL ─────────────────────────────────────────────────────
        const echoId=`echo-${uid}`;
        map.addSource(echoId,{type:'geojson',data:{type:'Feature',geometry:{type:'LineString',coordinates:fullArc}}});
        map.addLayer({id:echoId,type:'line',source:echoId,paint:{'line-color':color,'line-width':1,'line-opacity':0.35,'line-blur':2}});
        echoLayersRef.current.push(echoId);
        // Fade echo over 60s
        const echoStart=performance.now();
        const fadeEcho=(now)=>{
          const t=Math.min((now-echoStart)/60000,1);
          if(map.getLayer(echoId))map.setPaintProperty(echoId,'line-opacity',(1-t)*0.35);
          if(t<1)requestAnimationFrame(fadeEcho);
          else{if(map.getLayer(echoId))map.removeLayer(echoId);if(map.getSource(echoId))map.removeSource(echoId);echoLayersRef.current=echoLayersRef.current.filter(e=>e!==echoId);}
        };
    ...
    // ── Fade out main arc after 8s ──
    setTimeout(()=>{
      const fs=performance.now();
      const fo=(now)=>{
        const t=Math.min((now-fs)/800,1),op=1-t;
        ...
        else{[glowId,lineId,dotId,pulseId].forEach(id=>{if(map.getLayer(id))map.removeLayer(id);if(map.getSource(id))map.removeSource(id);});attackLayersRef.current=attackLayersRef.current.filter(l=>l.lineId!==lineId);}
      };
  ...
  useEffect(()=>{
    ...
    const map=new window.mapboxgl.Map({
      ...
      projection:'globe', zoom:1.8, center:[15,25], pitch:0, antialias:true
    });
    map.on('load',()=>{
      ...
      // Glow circles at country coords (heatmap effect using circle layer)
      map.addLayer({
        id:'country-heat',type:'circle',source:'heatmap-source',
        paint:{
          'circle-radius':['interpolate',['linear'],['get','intensity'],0,0,1,80],
          'circle-color':['interpolate',['linear'],['get','intensity'],0,'rgba(255,45,85,0)',0.3,'rgba(255,45,85,0.3)',1,'rgba(255,45,85,0.7)'],
          'circle-blur':1.5,
          'circle-opacity':['interpolate',['linear'],['get','intensity'],0,0,0.1,0.6,1,0.9],
          'circle-pitch-alignment':'map'
        }
      });
      // Outer softer glow
      map.addLayer({
        id:'country-heat-outer',type:'circle',source:'heatmap-source',
        paint:{
          'circle-radius':['interpolate',['linear'],['get','intensity'],0,0,1,140],
          'circle-color':'rgba(255,45,85,0.15)',
          'circle-blur':2.5,
          'circle-opacity':['interpolate',['linear'],['get','intensity'],0,0,0.2,0.4,1,0.6],
          'circle-pitch-alignment':'map'
        }
      });
  ...
  useEffect(()=>{
    ...
    const interval=setInterval(()=>{
      const a=generate();
      setAttacks(p=>[a,...p].slice(0,30));
      setStats(p=>({attacksToday:p.attacksToday+1,attacksActive:Math.floor(Math.random()*150)+50}));
      setTargetCounts(p=>({...p,[a.target.code]:(p[a.target.code]||0)+1}));
      setSourceCounts(p=>({...p,[a.source.code]:(p[a.source.code]||0)+1}));
      setTypeCounts(p=>({...p,[a.attackType]:(p[a.attackType]||0)+1}));
      apmBucketRef.current++;
      addAttackToMap(a);
    },800);

    // APM bucket: every 10s shift a new sample into history
    const apmInterval=setInterval(()=>{
      setApmHistory(p=>{const next=[...p.slice(1),apmBucketRef.current];apmBucketRef.current=0;return next;});
    },10000);

    return()=>{clearInterval(interval);clearInterval(apmInterval);};
  },[mapLoaded,addAttackToMap]);

  // ── Derived analytics ─────────────────────────────────────────────────────
  const top5Targets = countries
    .map(c=>({...c,count:targetCounts[c.code]||0}))
    .sort((a,b)=>b.count-a.count).slice(0,5).filter(c=>c.count>0);

  const top5Sources = countries
    .map(c=>({...c,count:sourceCounts[c.code]||0}))
    .sort((a,b)=>b.count-a.count).slice(0,5).filter(c=>c.count>0);
  ...
  return (
    <>
      ...
      <div style={{width:'100vw',height:'100vh',overflow:'hidden',background:'#04040f',fontFamily:"'Share Tech Mono',monospace"}}>
        ...
        {/* ── HEADER ── */}
        <header style={{position:'fixed',top:0,left:0,right:0,height:'68px',background:'linear-gradient(180deg,rgba(4,4,15,0.97) 0%,rgba(4,4,15,0.75) 100%)',backdropFilter:'blur(24px)',borderBottom:'1px solid rgba(0,200,255,0.15)',display:'flex',alignItems:'center',padding:'0 28px',zIndex:100,gap:'24px'}}>
          ...
          <div style={{marginLeft:'auto',display:'flex',gap:'20px',alignItems:'center'}}>
            ...
            {/* Globe/Flat toggle */}
            <button onClick={()=>setGlobeMode(g=>!g)} style={{padding:'6px 14px',borderRadius:'20px',background:globeMode?'rgba(0,200,255,0.1)':'rgba(123,45,255,0.1)',border:`1px solid ${globeMode?'rgba(0,200,255,0.4)':'rgba(123,45,255,0.4)'}`,color:globeMode?'#00c8ff':'#7b2dff',fontSize:'11px',cursor:'pointer',transition:'all 0.3s',fontFamily:"'Share Tech Mono',monospace",letterSpacing:'1px'}}>
              {globeMode?'🌐 GLOBE':'🗺️ FLAT'}
            </button>
            {/* Stats panel toggle */}
            <button onClick={()=>setShowStats(s=>!s)} style={{padding:'6px 14px',borderRadius:'20px',background:showStats?'rgba(255,214,10,0.15)':'rgba(0,200,255,0.05)',border:`1px solid ${showStats?'rgba(255,214,10,0.5)':'rgba(0,200,255,0.2)'}`,color:showStats?'#ffd60a':'#4a5568',fontSize:'11px',cursor:'pointer',transition:'all 0.3s',fontFamily:"'Share Tech Mono',monospace",letterSpacing:'1px'}}>
              📊 ANALYTICS
            </button>
        ...
        {/* ── MAP ── */}
        <div ref={mapRef} style={{position:'fixed',top:'68px',left:0,right:selectedAttack?'500px':showStats?'380px':0,bottom:0,transition:'right 0.4s cubic-bezier(0.16,1,0.3,1)'}}/>

        {/* ── LIVE FEED ── */}
        <div style={{position:'fixed',bottom:'20px',left:'20px',width:'290px',maxHeight:'250px',overflowY:'auto',background:'rgba(6,8,20,0.92)',backdropFilter:'blur(20px)',border:'1px solid rgba(0,200,255,0.15)',borderRadius:'12px',padding:'14px',zIndex:50}}>
          ...
          {attacks.map(a=>(
            <div key={a.id} onClick={()=>{setSelectedAttack(a);setActiveTab('overview');}} style={{padding:'9px 11px',background:'rgba(0,200,255,0.03)',borderLeft:`3px solid ${getSeverityColor(a.severity)}`,borderRadius:'6px',marginBottom:'7px',fontSize:'11px',cursor:'pointer',transition:'all 0.2s',animation:'slideIn 0.4s ease'}}
        ...
        {/* ── LEGEND ── */}
        <div style={{position:'fixed',bottom:'20px',right:selectedAttack?'520px':showStats?'400px':'20px',background:'rgba(6,8,20,0.92)',backdropFilter:'blur(20px)',border:'1px solid rgba(0,200,255,0.15)',borderRadius:'12px',padding:'12px 16px',zIndex:50,transition:'right 0.4s cubic-bezier(0.16,1,0.3,1)',fontSize:'11px'}}>
          ...
          {[['critical','#ff2d55'],['high','#ff6b35'],['medium','#ffd60a'],['low','#30d158']].map(([label,color])=>(
            <div key={label} style={{display:'flex',alignItems:'center',gap:'7px',marginBottom:'7px'}}>
              <div style={{width:'24px',height:'2px',background:color,boxShadow:`0 0 5px ${color}`,borderRadius:2}}/>
              <div style={{width:6,height:6,borderRadius:'50%',background:color,boxShadow:`0 0 7px ${color}`}}/>
              <span style={{color:'#718096',textTransform:'uppercase',letterSpacing:'1px',fontSize:'10px'}}>{label}</span>
            </div>
          ))}
        </div>
        ...
        {showStats && !selectedAttack && (
          <div style={{position:'fixed',right:0,top:'68px',bottom:0,width:'380px',background:'rgba(5,6,18,0.97)',backdropFilter:'blur(24px)',borderLeft:'1px solid rgba(0,200,255,0.12)',overflowY:'auto',zIndex:90,animation:'slideInRight 0.35s cubic-bezier(0.16,1,0.3,1)',padding:'20px'}}>
            <div style={{fontFamily:'Orbitron',fontSize:'14px',color:'#00c8ff',letterSpacing:'3px',marginBottom:'20px',paddingBottom:'10px',borderBottom:'1px solid rgba(0,200,255,0.1)'}}>📊 ANALYTICS</div>

            {/* Attacks per minute sparkline */}
            ...
            <div style={{padding:'14px',background:'rgba(0,200,255,0.04)',border:'1px solid rgba(0,200,255,0.1)',borderRadius:'10px',marginBottom:'18px'}}>
              {Object.keys(typeCounts).length > 0
                ...
                : <div style={{color:'#4a5568',fontSize:'11px',textAlign:'center',padding:'16px 0'}}>Waiting for data...</div>
              }
            </div>
            ...
            <div style={{padding:'14px',background:'rgba(255,45,85,0.04)',border:'1px solid rgba(255,45,85,0.12)',borderRadius:'10px',marginBottom:'18px'}}>
              {top5Targets.length > 0
                ? <BarChart data={top5Targets}/>
                : <div style={{color:'#4a5568',fontSize:'11px',textAlign:'center',padding:'16px 0'}}>Waiting for data...</div>
              }
            </div>

            {/* Most active attacker leaderboard */}
                ...
                <div>
                  {top5Sources.map((c,i)=>(
                    <div key={c.code} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 0',borderBottom:i<top5Sources.length-1?'1px solid rgba(0,200,255,0.06)':'none'}}>
                      <span style={{fontFamily:'Orbitron',fontSize:'11px',color:i===0?'#ffd60a':i===1?'#718096':i===2?'#ff6b35':'#4a5568',width:'18px',flexShrink:0}}>#{i+1}</span>
                      <span style={{fontSize:'16px'}}>{c.flag}</span>
                      <span style={{color:'#a0aec0',fontSize:'11px',flex:1}}>{c.name}</span>
                      <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                        <div style={{width:`${Math.round((c.count/top5Sources[0].count)*50)}px`,height:'4px',background:'linear-gradient(90deg,#7b2dff,#ff2d55)',borderRadius:'2px',minWidth:'4px'}}/>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
              ...
              {activeTab==='killchain'&&(
                <div>
                  ...
                  <p style={{color:'#4a5568',fontSize:'11px',marginBottom:'18px',lineHeight:1.7}}>The kill chain maps every phase an attacker goes through. Defenders who understand each phase can interrupt the attack before it reaches the end.</p>
                  <div style={{position:'relative'}}>
                    {info.killChain.map((step,i)=>(
                      <div key={i} style={{display:'flex',gap:'14px',marginBottom:'4px',position:'relative'}}>
                        ...
                        <div style={{flexShrink:0,width:'34px',height:'34px',borderRadius:'50%',background:i===info.killChain.length-1?'rgba(255,45,85,0.2)':'rgba(0,200,255,0.1)',border:`2px solid ${i===info.killChain.length-1?'#ff2d55':'rgba(0,200,255,0.4)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',zIndex:1,boxShadow:i===info.killChain.length-1?'0 0 14px rgba(255,45,85,0.4)':'0 0 8px rgba(0,200,255,0.2)'}}>{step.icon}</div>
              ...
              {activeTab==='mitre'&&(
                <div>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'8px',marginBottom:'20px'}}>
                    <MitreCard label="TACTIC" id={info.mitre.tacticId} name={info.mitre.tactic} desc="The adversary's high-level goal — what they are ultimately trying to achieve in this phase" color="#7b2dff"/>
                    ...
                    {info.mitre.subId!==info.mitre.techniqueId&&<>
                      ...
                      <MitreCard label="SUB-TECHNIQUE" id={info.mitre.subId} name={info.mitre.subTechnique} desc="A more specific implementation of the parent technique used in this attack" color="#30d158"/>
                    </>}
                  </div>
                  ...
                  <div style={{padding:'13px 15px',background:'rgba(123,45,255,0.06)',border:'1px solid rgba(123,45,255,0.2)',borderRadius:'8px'}}>
                    ...
                    <p style={{color:'#718096',fontSize:'12px',margin:0,lineHeight:1.8}}>By mapping attacks to ATT&CK IDs, security teams can build precise detection rules in SIEMs, identify coverage gaps, and benchmark their posture against real-world adversary behaviour observed across thousands of incidents.</p>
                  </div>
                </div>
              )}
              ...
              {activeTab==='examples'&&(
                <div>
                  ...
                  <p style={{color:'#4a5568',fontSize:'11px',marginBottom:'18px',lineHeight:1.7}}>Documented real attacks using this exact technique — understanding the real-world scale makes the threat concrete.</p>
              ...
              {activeTab==='defend'&&(
                <div>
                  ...
                  <p style={{color:'#4a5568',fontSize:'11px',marginBottom:'14px',lineHeight:1.7}}>Select your level to see the most relevant defences. Layering multiple controls dramatically reduces risk.</p>
                  ...
                  <div style={{padding:'13px 15px',background:'rgba(255,214,10,0.04)',border:'1px solid rgba(255,214,10,0.18)',borderRadius:'8px'}}>
                    ...
                    <p style={{color:'#718096',fontSize:'12px',margin:0,lineHeight:1.8}}>No single control is sufficient. Layer multiple mitigations — patch management, MFA, and backups alone stop the vast majority of attacks.</p>
                  </div>
                </div>
              )}
            </div>
